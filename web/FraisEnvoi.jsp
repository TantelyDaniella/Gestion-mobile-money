...
<%@ page import="java.util.*, Model.FraisEnvoi" %>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Gestion des frais d'envoi</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="topbar">
        ...
        <h1>💰 Gestion des frais d'envoi</h1>
    </div>
                ...
                <table class="data-table">
                    <thead>
                        <tr>
                            ...
                            <th>Frais d'envoi (Ar)</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        <%
                            List<FraisEnvoi> fraisList = (List<FraisEnvoi>) request.getAttribute("listeFraisEnvoi");
                            if (fraisList != null && !fraisList.isEmpty()) {
                                for (FraisEnvoi f : fraisList) {
                        %>
                        <tr>
                            <td><%= f.getIdEnv() %></td>
                            <td><%= String.format("%,d", f.getMontant1()) %></td>
                            <td><%= String.format("%,d", f.getMontant2()) %></td>
                            <td><%= String.format("%,d", f.getFraisEnv()) %></td>
                            <td class="actions">
                        ...
                        <% }} else { %>
                        <tr><td colspan="5" style="text-align: center; padding: 2rem;">Aucun barème de frais d'envoi trouvé</td></tr>
                        <% } %>
                    </tbody>
                </table>
            </div>
    ...
    <div id="form-card">
        ...
        <form class="frais-form" method="post" action="FraisEnvoiServlet" id="frais-form">
            ...
            <div class="row">
                <label for="fraisenv">Frais d'envoi (Ar) *</label>
                <input type="number" id="fraisenv" name="fraisenv" required min="0" step="1">
            </div>
    ...
    <div class="footer">
        <p>© 2026 Mobile Money - Gestion des frais d'envoi</p>
    </div>
    ...
    <script>
        ...
        function showForm(type, btn = null) {
            ...
            if (type === "add") {
                formTitle.textContent = "Ajouter un barème de frais d'envoi";
                formAction.value = "ajouter";
                document.getElementById("frais-form").reset();
                document.getElementById("idenv").value = "";
                currentAction = 'add';
            } else if (type === "edit" && btn) {
                const row = btn.closest("tr");
                formTitle.textContent = "Modifier un barème de frais d'envoi";
                formAction.value = "modifier";

                const idenv = row.cells[0].textContent.trim();
                ...
                const fraisenv = row.cells[3].textContent.trim().replace(/,/g, '').replace(/\s/g, '');

                document.getElementById("idenv").value = idenv;
                document.getElementById("montant1").value = montant1;
                document.getElementById("montant2").value = montant2;
                document.getElementById("fraisenv").value = fraisenv;
                currentAction = 'edit';
            }
        }
        ...
        function confirmDelete(btn) {
            ...
            if (confirm("Voulez-vous vraiment supprimer le barème : " + montant1 + " Ar - " + montant2 + " Ar ?")) {
                window.location.href = "FraisEnvoiServlet?action=supprimer&idenv=" + encodeURIComponent(idenv);
            }
        }
        ...
        document.getElementById("frais-form").addEventListener("submit", function(e) {
            ...
            const fraisenv = parseInt(document.getElementById("fraisenv").value);
            ...
            if (isNaN(fraisenv) || fraisenv < 0) {
                e.preventDefault();
                showTempMessage("Les frais d'envoi doivent être un nombre positif", "error");
                return;
            }
        });
